...
Derivada da tela de loja (`screen-store-0010.md`) e dos requisitos de gestão de produtos, categorias, imagens, promoções, estoque e variações.
...
**Descrição da tela:**  
Tela usada pelo lojista para cadastrar um produto no catálogo da sua loja, informando dados comerciais, categoria específica opcional, imagens, disponibilidade, promoção, estoque e variações do produto.
...
`RF003`, `RF008`, `RF019`, `RF020`, `RF022`, `RF023`, `RF024`, `RF025`, `RF026`, `RF027`, `RF028`, `RF031`, `RF032`
...
- Conteúdo: formulário dividido em seções para dados básicos, categoria, imagens, preço/promoção, estoque e variações.
...
| Campo | Tipo | Obrigatório | Placeholder | Validação | Observação |
| --- | --- | --- | --- | --- | --- |
| Produto em destaque | Toggle | Não | Não se aplica | Booleano | Habilita exibição em área de destaque quando disponível |
| Produto em promoção | Toggle condicional | Não | Não se aplica | Só deve ser usado quando destaque estiver habilitado | Promoção deve respeitar regras do produto |
...
| Campo | Tipo | Obrigatório | Placeholder | Validação | Observação |
| --- | --- | --- | --- | --- | --- |
| Tipo de estoque | Segmented control ou select | Sim | `Estoque livre` | Deve ser `Livre` ou `Controlado` | Requisito funcional prevê controle por produto |
| Quantidade inicial | Número inteiro | Condicional | `0` | Obrigatório quando estoque for controlado | Se chegar a zero, compra deve ser bloqueada |

**Pendência de contrato:**  
Os requisitos funcionais mencionam estoque livre e estoque controlado, porém o modelo analisado do PR de banco não apresentou claramente campos como `stock_mode` ou `stock_quantity` em `products`. A tela deve prever a UX e os tipos de frontend, mas o contrato final com backend precisa ser validado antes da integração real.
...
- Estoque controlado deve exigir quantidade inicial quando o contrato backend estiver definido.
...
8. O lojista pode definir disponibilidade, destaque, promoção e estoque.
...
- Estoque ainda precisa de confirmação de contrato no backend.

## Design

**Layout:**  
Tela de gestão, mobile-first, com formulário dividido em blocos claros. Campos obrigatórios devem aparecer primeiro. Configurações avançadas, como promoção, estoque e variações, podem ficar em seções expansíveis para não pesar o cadastro básico.
...
**Tipografia:**  
Título forte no topo. Labels claros acima dos campos. Textos auxiliares menores para explicar categoria `Todos`, preço em reais, variações e estoque.

**Espaçamentos:**  
Separar visualmente dados básicos, categoria, imagens, promoção, estoque e variações. Blocos de variação devem ter espaçamento suficiente para opções dinâmicas sem confundir o lojista.

**Componentes reutilizáveis:**  
`ProductForm`, `ProductBasicInfoSection`, `ProductCategorySection`, `ProductImageUploader`, `ProductPromotionSection`, `ProductStockSection`, `ProductVariationSection`, `VariationOptionRow`, `MoneyInput`, `CategorySelect`, `AvailabilityToggle`, `FormActions`.

**Ícones:**  
Produto, imagem, categoria, tag/promoção, estoque, variação, adicionar, remover, salvar e voltar.
...
O estoque aparece nos requisitos funcionais, mas precisa de validação de contrato com o backend antes da integração final.
